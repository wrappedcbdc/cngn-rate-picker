...
import { MexcProvider } from "../src/providers/mexc.js";
...
/** A MEXC kline `[openTime, o, h, l, c, vol, closeTime, quoteVol]`. */
function mexcCandle(close: number, minutesAgo: number) {
  const open = Date.now() - minutesAgo * 60_000;
  return [open, String(close), String(close), String(close), String(close), "100", open + 60_000, "100"];
}

/** Serves klines, bookTicker, and ticker/price off one fake fetch. */
function mexcFetch(
  candles: unknown,
  book: unknown = { symbol: "USDTNGN", bidPrice: "1394.0", bidQty: "10", askPrice: "1396.0", askQty: "10" },
  last: unknown = { symbol: "USDTNGN", price: "1395.0" },
  capture?: (url: string) => void,
): typeof fetch {
  return (async (input: RequestInfo | URL) => {
    const url = String(input);
    capture?.(url);
    if (url.includes("/klines")) return new Response(JSON.stringify(candles), { status: 200 });
    if (url.includes("/ticker/bookTicker")) return new Response(JSON.stringify(book), { status: 200 });
    return new Response(JSON.stringify(last), { status: 200 });
  }) as typeof fetch;
}

test("MexcProvider defaults to a TWAP of the USDTNGN kline", async () => {
  let url = "";
  // 1600 stood for 30 min, then 1562.5 for 20. Edges kept clear of the window.
  const candles = [mexcCandle(1600, 50), mexcCandle(1562.5, 20)];
  const provider = new MexcProvider();
  const quote = await provider.getPriceInNgn(
    fakeCtx(mexcFetch(candles, undefined, undefined, (u) => (url = u))),
  );

  assert.equal(provider.asset, "USDT");
  assert.match(url, /\/api\/v3\/klines\?symbol=USDTNGN&interval=1m&limit=61$/);
  const expected = (1600 * 30 + 1562.5 * 20) / 50;
  assert.ok(Math.abs(quote.price - expected) < 5, `expected ~${expected}, got ${quote.price}`);
});

test("MexcProvider price: last reads /ticker/price", async () => {
  const urls: string[] = [];
  const provider = new MexcProvider({ price: "last" });
  const quote = await provider.getPriceInNgn(
    fakeCtx(mexcFetch([], undefined, undefined, (u) => urls.push(u))),
  );
  assert.match(urls[0]!, /\/api\/v3\/ticker\/price\?symbol=USDTNGN$/);
  assert.equal(quote.price, 1395);
});

test("MexcProvider price: mid/bid/ask read /ticker/bookTicker", async () => {
  const urls: string[] = [];
  const mid = await new MexcProvider({ price: "mid" }).getPriceInNgn(
    fakeCtx(mexcFetch([], undefined, undefined, (u) => urls.push(u))),
  );
  const bid = await new MexcProvider({ price: "bid" }).getPriceInNgn(fakeCtx(mexcFetch([])));
  const ask = await new MexcProvider({ price: "ask" }).getPriceInNgn(fakeCtx(mexcFetch([])));

  assert.match(urls[0]!, /\/api\/v3\/ticker\/bookTicker\?symbol=USDTNGN$/);
  assert.equal(mid.price, 1395);
  assert.equal(bid.price, 1394);
  assert.equal(ask.price, 1396);
});

test("MexcProvider falls back to the book mid on an empty window", async () => {
  const quote = await new MexcProvider().getPriceInNgn(fakeCtx(mexcFetch([])));
  assert.equal(quote.price, 1395);
  assert.equal((quote.raw as { twapFellBackTo?: string }).twapFellBackTo, "mid");
});

test("MexcProvider twapFallback: false throws on an empty window", async () => {
  const provider = new MexcProvider({ twapFallback: false });
  await assert.rejects(
    () => provider.getPriceInNgn(fakeCtx(mexcFetch([]))),
    /no MEXC candles for "USDTNGN" in the last 3600000ms/,
  );
});

test("MexcProvider surfaces MEXC's error code and message", async () => {
  const fetchImpl = (async () =>
    new Response(JSON.stringify({ msg: "invalid symbol", code: -1121 }), {
      status: 400,
  ...
  await assert.rejects(
    () => provider.getPriceInNgn(fakeCtx(fetchImpl)),
    /MEXC -1121: invalid symbol for "USDTNGN"/,
  );
});

test("MexcProvider rejects a stale kline series", async () => {
  const provider = new MexcProvider();
  await assert.rejects(
    () => provider.getPriceInNgn(fakeCtx(mexcFetch([mexcCandle(1600, 60 * 24 * 30)]))),
    /is stale: newest candle is \d+\.\dh old/,
  );
});

test("MexcProvider builds the symbol from asset and quote, and infers inversion", async () => {
  const urls: string[] = [];
  const usdc = new MexcProvider({ asset: "USDC" });
  await usdc.getPriceInNgn(fakeCtx(mexcFetch([mexcCandle(1600, 30)], undefined, undefined, (u) => urls.push(u))));
  assert.match(urls[0]!, /symbol=USDCNGN&/);

  // A reversed symbol quotes asset per NGN, so it needs inverting.
  const reversed = new MexcProvider({ symbol: "NGNUSDT" });
  const quote = await reversed.getPriceInNgn(fakeCtx(mexcFetch([mexcCandle(0.000625, 30)])));
  assert.equal(quote.price, 1 / 0.000625);
});

test("MexcProvider quote option targets a non-NGN market", async () => {
  const urls: string[] = [];
  const provider = new MexcProvider({ quote: "USDT", asset: "USDC", price: "last" });
  await provider.getPriceInNgn(
    fakeCtx(mexcFetch([], undefined, { symbol: "USDCUSDT", price: "1.00094" }, (u) => urls.push(u))),
  );
  assert.match(urls[0]!, /symbol=USDCUSDT$/);
});

test("MexcProvider klineInterval scales the requested candle count", async () => {
  const urls: string[] = [];
  const provider = new MexcProvider({ klineInterval: "5m" });
  await provider.getPriceInNgn(
    fakeCtx(mexcFetch([mexcCandle(1600, 30)], undefined, undefined, (u) => urls.push(u))),
  );
  assert.match(urls[0]!, /interval=5m&limit=13$/); // 60/5 + 1
});

test("MexcProvider rejects a nonsensical TWAP window", () => {
  assert.throws(() => new MexcProvider({ twapWindowMs: 0 }), /twapWindowMs must be a positive/);
});
